---
name: cpp-crash-debug
description: |
  C++ crash diagnosis and core dump analysis with GDB via mcp-dap-server. Diagnose segfaults, aborts, stack overflows, memory corruption, and use-after-free bugs.
  TRIGGER when: C++ program crashes, analyzing core dumps, investigating segfaults, debugging aborts, diagnosing memory corruption, or performing post-mortem C++ analysis.
  DO NOT TRIGGER when: program is running normally (use cpp-debug), debugging Go programs, or no crash/core dump exists yet.
---
...
| SIGSTACK | Stack overflow | Infinite recursion, large stack allocation |
...
**Fix**: Use smart pointers, null after delete, check ownership.
...
**Fix**: Use std::array, std::vector, bounds checking.
...
- **mcp-dap-server**: `debug`, `context`, `evaluate`, `info`, `breakpoint`
